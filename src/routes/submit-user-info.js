...
export const submitUserInfo = {
    ...
    handler: async (req, res)=>{
        try {
            const { user_id, dob, gender, weight, height, diet, activity, weightGoal, activityGoal } = req.body;
            ...
            jwt.verify(token, process.env.JWT_SECRET, async (err, decoded)=>{
                ...
                const [ result ] = await db.query(`UPDATE users SET new_user = FALSE, dob = "${dob}", gender = "${gender}", weight = ${weight}, height = ${height}, dietary_preference = "${diet}", activity_level = "${activity}", weight_goal = ${weightGoal}, activity_goal = "${activityGoal}" WHERE user_id = '${user_id}'`);
