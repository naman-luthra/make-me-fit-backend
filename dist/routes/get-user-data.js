...
var getUserData = {
  ...
  handler: function () {
    var _handler = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(req, res) {
      var user_id, authorization, token;
                ...
                return _regeneratorRuntime().wrap(function _callee$(_context) {
                  while (1) switch (_context.prev = _context.next) {
                    ...
                    case 7:
                      db = _context.sent;
                      _context.next = 10;
                      return db.query("SELECT image, dob, gender, weight, height, dietary_preference, activity_level, weight_goal, activity_goal, active_plan_id, new_user FROM users WHERE user_id = '".concat(user_id, "'"));
                    case 10:
                      _yield$db$query = _context.sent;
                      _yield$db$query2 = _slicedToArray(_yield$db$query, 1);
                      resultUsers = _yield$db$query2[0];
                    ...
                    case 15:
                      _context.next = 17;
                      return db.query("SELECT weight, water, excercise, food FROM tracking_history WHERE user_id = '".concat(user_id, "' and date = CURDATE()"));
                    case 17:
                      _yield$db$query3 = _context.sent;
                      _yield$db$query4 = _slicedToArray(_yield$db$query3, 1);
                      resultHistory = _yield$db$query4[0];
                      todayHistory = resultHistory.length === 0 ? null : {
                      ...
                      _context.next = 23;
                      return db.query("SELECT fitness_plans.plan_id as plan_id, fitness_plans.plan_name as plan_name FROM user_fitness_plans INNER JOIN fitness_plans ON fitness_plans.plan_id = user_fitness_plans.plan_id WHERE user_id = '".concat(user_id, "'"));
                    case 23:
                      _yield$db$query5 = _context.sent;
                      _yield$db$query6 = _slicedToArray(_yield$db$query5, 1);
                      resultPlans = _yield$db$query6[0];
                    ...
                    case 28:
                      _context.next = 30;
                      return db.query("SELECT plan_name as active_fitness_plan_name, active_diet_plan_id, active_routine_id FROM fitness_plans WHERE plan_id = ".concat(resultUsers[0].active_plan_id));
                    case 30:
                      _yield$db$query7 = _context.sent;
                      _yield$db$query8 = _slicedToArray(_yield$db$query7, 1);
                      _yield$db$query8$ = _slicedToArray(_yield$db$query8[0], 1);
                      _yield$db$query8$$ = _yield$db$query8$[0];
                      active_fitness_plan_name = _yield$db$query8$$.active_fitness_plan_name;
                      active_diet_plan_id = _yield$db$query8$$.active_diet_plan_id;
                      active_routine_id = _yield$db$query8$$.active_routine_id;
                      _context.next = 39;
                      return db.query("SELECT fitness_plan_diet_plans.diet_plan_id as diet_plan_id, data as diet_plan_data, diet_plans.plan_name as plan_name FROM fitness_plan_diet_plans INNER JOIN diet_plans ON fitness_plan_diet_plans.diet_plan_id=diet_plans.plan_id WHERE fitness_plan_diet_plans.plan_id = ".concat(resultUsers[0].active_plan_id));
                    case 39:
                      _yield$db$query9 = _context.sent;
                      _yield$db$query10 = _slicedToArray(_yield$db$query9, 1);
                      dietPlans = _yield$db$query10[0];
                      _context.next = 44;
                      return db.query("SELECT fitness_plan_workout_routines.routine_id as routine_id, data as routine_data, workout_routines.routine_name as routine_name FROM fitness_plan_workout_routines INNER JOIN workout_routines ON fitness_plan_workout_routines.routine_id=workout_routines.routine_id WHERE fitness_plan_workout_routines.plan_id = ".concat(resultUsers[0].active_plan_id));
                    case 44:
                      _yield$db$query11 = _context.sent;
                      _yield$db$query12 = _slicedToArray(_yield$db$query11, 1);
                      routines = _yield$db$query12[0];
                      mealPlans = dietPlans.map(function (_ref2) {
                        var diet_plan_id = _ref2.diet_plan_id,
                          plan_name = _ref2.plan_name,
                          diet_plan_data = _ref2.diet_plan_data;
                        return {
                          id: diet_plan_id,
                          name: plan_name,
                      ...
                      workoutRoutines = routines.map(function (_ref3) {
                        var routine_id = _ref3.routine_id,
                          routine_name = _ref3.routine_name,
                          routine_data = _ref3.routine_data;
                        return {
                          id: routine_id,
                          name: routine_name,
